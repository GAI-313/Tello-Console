...
## curve コマンドの動作について
　curve コマンドは、始点座標（x1, y1, z1）と終点座標（x2, y2, z2）をもとに以下のような正円（実際は3次元に移動するので球体となる）の円周を軌道として終点座標へ飛行します。始点座標はドローンの非公開7点ではなく、あくまで円周軌道作成のベースとして機能します。この2つのパラメータによってシミュレートされた球体面上から離れたポジションに終点座標が定義された場合、エラーが発生します。
