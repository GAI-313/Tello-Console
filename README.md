...
### sample_boot.py
　このサンプルコードは、Tello-Console を使用するための最も基礎を学部ことができるコードです。このコードには、以下の内容が記述されています。
```python
# -*- coding: utf-8 -*-
"""
sample_boot.py
...
```
　大半がコメント文を占めていますが、コメント文を省くと以下のように構成されています。
```python
# -*- coding: utf-8 -*-
...
- **config: utf-8** : このコメント文をプログラム上部に記述することで日本語がシステム内で認識されるようになります。最近では必要ないと言われていますが書いて損はないです。
- **from modules.tello import console** : これは import 文といい、他のプログラムを読み込み際に使用するコードです。このコードは、以下のように分解することができます。
    - **from modules.tello** : これは modules フォルダ内の tello.py というファイルを参照する。という意味です。
    - **import console** : これはこのコードの前に記述された参照元にある console というものを持ってきます。という意味です。
 つまり、このコードの意味は、**modules フォルダにある tello.py の中にある console クラスを持ってきます。** という意味になります。
- **drone = console()** : これは、drone という変数に console クラスを代入している状態です。このように任意のクラスを任意の変数に格納することで、実行時に格納されたクラスが活性化(インスタンス化)し、クラス内に記述された **__init__（イニャライザ）メソッド** が実行され、初期動作が行われます。console クラスのイニャライザには、以下の処理が行われます。
    1. パソコンとドローンとのUDP通信接続を行う。
    2. ドローンが以降のプログラムを実行できる状態かどうか診断する。
    3. ドローンからカメラデータの受信とその他応答データの受信を開始。
    4. ドローンとパソコン間の通信が安定するように待機、調整する。
    5. セットアップが完了したことを出力する。

 この2行の記述により、Tello-Console の本体である console クラスを呼び出し、任意の変数に格納することで初期動作を実行させてドローンをプログラムできる状態にするのです。これを基礎として理解できれば、あとは任意のメソッドを呼び出してドローンのプログラムを作成できます。

### sample_flight_1.py
　このサンプルコードは、ドローンを離着陸させる簡易的なプログラムの方法を学習できます。これを理解することで、ドローンを操作するためのコードの記述方法を理解することができます。このサンプルコードは、コメント文を除くと以下のように構成されています。
```python
# -*- coding: utf-8 -*- 
